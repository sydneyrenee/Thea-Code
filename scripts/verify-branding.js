...
try {
    ...
    if (!fs.existsSync(extensionDir)) {
         console.warn(`Warning: Expected 'extension' directory not found in extracted VSIX at ${tempDirPath}. Searching root.`);
...
} finally { // --- 5. Cleanup temporary directory ---
}
...
function scanDirectory(dirPath) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (!['.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg', '.ttf', '.woff', '.woff2', '.eot', '.vsix'].includes(ext)) {
                try {
