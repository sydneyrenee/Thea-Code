...
import type { Content, Part, TextPart, InlineDataPart, FunctionCallPart, FunctionResponsePart } from "@google/generative-ai"; // Import Gemini types
...
export function convertToGeminiHistory(
...
): Content[] { // Gemini history is an array of Content objects
    return neutralHistory.map(neutralMessage => {
        ...
        if (typeof neutralMessage.content === 'string') {
            ...
            geminiMessage.parts = neutralMessage.content.flatMap(block => {
                if (block.type === 'text') {
 ...
 */
export function convertToGeminiContentBlocks(
    neutralContent: NeutralMessageContent
): Part[] { // Gemini content is an array of Part objects
