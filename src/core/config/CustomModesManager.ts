...
export class CustomModesManager {
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		this.disposables.push(
			vscode.workspace.onDidSaveTextDocument(async (document) => {
				if (arePathsEqual(document.uri.fsPath, settingsPath)) {
					...
					const theamodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []

					// Merge modes from both sources (.thea_modes takes precedence)
					const mergedModes = await this.mergeCustomModes(theamodesModes, result.data.customModes)
		...
		const theamodesPath = await this.getWorkspaceRoomodes()
		if (theamodesPath) {
			this.disposables.push(
				vscode.workspace.onDidSaveTextDocument(async (document) => {
					if (arePathsEqual(document.uri.fsPath, theamodesPath)) {
						const settingsModes = await this.loadModesFromFile(settingsPath)
						const theamodesModes = await this.loadModesFromFile(theamodesPath)
						// .thea_modes takes precedence
						const mergedModes = await this.mergeCustomModes(theamodesModes, settingsModes)
						await this.context.globalState.update("customModes", mergedModes)
						await this.onUpdate()
					}
				}),
	...
	async getCustomModes(): Promise<ModeConfig[]> {
		...
		const theamodesPath = await this.getWorkspaceRoomodes()
		const theamodesModes = theamodesPath ? await this.loadModesFromFile(theamodesPath) : []
		...
		for (const mode of theamodesModes) {
		...
		const mergedModes = [
			...theamodesModes.map((mode) => ({ ...mode, source: "project" as const })),
			...settingsModes
	...
	async deleteCustomMode(slug: string): Promise<void> {
		try {
			...
			const projectMode = theamodesModes.find((m) => m.slug === slug)
