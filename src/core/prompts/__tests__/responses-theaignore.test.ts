...
describe("TheaIgnore Response Formatting", () => {
	...
	describe("formatResponse.theaIgnoreError", () => {
		...
		it("should format error message for ignored files", () => {
			...
			expect(errorMessage).toContain(`Access to secrets/api-keys.json is blocked by the ${GLOBAL_FILENAMES.IGNORE_FILENAME} file settings`) // Use constant
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain(`ask the user to update the ${GLOBAL_FILENAMES.IGNORE_FILENAME} file`) // Use constant
		})
