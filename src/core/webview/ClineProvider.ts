...
import { VIEWS, EXTENSION_DISPLAY_NAME, configSection } from "../../../dist/thea-config"
...
export class ClineProvider extends EventEmitter<ClineProviderEvents> implements vscode.WebviewViewProvider {
	...
	private async generateSystemPrompt(message: WebviewMessage) {
		...
		const rooIgnoreInstructions = this.getCurrentCline()?.theaIgnoreController?.getInstructions()
	...
	public static async handleCodeAction(
		...
		// Check if command exists and endsWith is a valid function before calling
		if (
			visibleProvider.getCurrentCline() &&
			command &&
			typeof command.endsWith === "function" &&
	...
	public static async handleTerminalAction(
		...
		if (promptType === "TERMINAL_ADD_TO_CONTEXT") {
			// Use promptType for clarity
			await visibleProvider.postMessageToWebview({
				type: "invoke",
				invoke: "setChatBoxMessage",
				text: prompt,
	...
	public async initClineWithSubTask(parent: Cline, task?: string, images?: string[]) {
		...
		if (this.view) {
			// Add null check
			if ("onDidChangeViewState" in this.view) {
				// Replace webviewView with this.view
				this.view.onDidChangeViewState(
					// Replace webviewView with this.view
					() => {
			...
			} else if ("onDidChangeVisibility" in this.view) {
				// Replace webviewView with this.view
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		html = html
			.replace(/src="(?!http)([^"]+)"/g, (match: string, src: string) => {
				// Added types
				// Original lines: ~654-737 (getHtmlContent structure changed)
				return `src="${proxyUrl}/${src}"`
			})
			.replace(/href="(?!http)([^"]+)"/g, (match: string, href: string) => {
				// Added types
		...
		webview.onDidReceiveMessage(
			async (message: WebviewMessage) => {
				...
				switch (type) {
					...
					case "checkoutDiff":
						{
							// Reverted to original logic structure
					...
					case "refreshGlamaModels": {
						...
						if (Object.keys(glamaModels).length > 0) {
							...
							await this.cacheManager.writeModelsToCache(
								// Use cacheManager
					...
					case "openMcpSettings": {
						// From original (was in handleWebviewAction via openMcpSettingsFile)
					...
					case "restartMcpServer": {
						// Already delegated via restartMcpConnection
						try {
					...
					case "toggleToolAlwaysAllow": {
						// Already delegated
						try {
							await this.mcpManager.toggleToolAlwaysAllow(
								// Use manager
								message.serverName!,
								message.toolName!,
								message.alwaysAllow!,
					...
					case "toggleMcpServer": {
						// Already delegated via toggleMcpServerDisabled
						try {
					...
					case "updateExperimental": {
						...
						const updatedExperiments = {
							...((await this.getGlobalState("experiments")) ?? {}),
							...message.values,
					...
					case "humanRelayResponse": {
						...
						if (message.requestId && message.text) {
							vscode.commands.executeCommand("thea-code.handleHumanRelayResponse", {
								// Use branded command
					...
					case "searchCommits": {
						...
						await this.postMessageToWebview({
							...
							gitSearchResults: commitResults,
					...
					case "showRooIgnoredFiles": // Keep the message type name for now
						...
						await this.updateGlobalState(
							SETTING_KEYS.SHOW_IGNORED_FILES as GlobalStateKey,
							message.bool ?? true,
						)
						await this.postStateToWebview()
						break
	...
	private async handleWebviewAction(action: string, data: any) {
		...
		switch (action) {
			...
			case "openRouter":
				...
				const openRouterUrl =
					"https://openrouter.ai/auth?callback_url=vscode://sydneyrenee.thea-code/auth/openrouter"
	...
	async getStateToPostToWebview(): Promise<ExtensionState | undefined> {
		...
		if (!state || !state.mode) {
			this.outputChannel.appendLine(
				"[getStateToPostToWebview] State retrieval failed or returned minimal fallback. Returning undefined.",
			)
			return undefined
