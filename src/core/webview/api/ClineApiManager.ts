...
export class ClineApiManager {
     ...
     * Note: Does not update the active Cline instance's API handler directly. That is the responsibility of the calling ClineProvider.
     */
    async updateApiConfiguration(apiConfiguration: ApiConfiguration): Promise<void> {
        ...
        // Removed updating current Cline's API handler - ClineProvider should do this
    }
     ...
     */
    public async handleModeSwitch(newMode: Mode): Promise<ApiConfiguration | undefined> { // Return the config to load
        // Telemetry capture removed - should be handled by ClineProvider which has taskId
        ...
        // Removed postStateToWebview - ClineProvider should do this
    ...
    async handleOpenRouterCallback(code: string): Promise<void> {
        ...
        // Provider will call postStateToWebview
    }
