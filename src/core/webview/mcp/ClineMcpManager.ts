...
export class ClineMcpManager {
    ...
    constructor(
        ...
        // McpHub instance is injected via setMcpHub by ClineProvider
    }

    /**
     * Sets the shared McpHub instance. Called by ClineProvider.
     */
    ...
    async ensureMcpServersDirectoryExists(): Promise<string> {
        // Logic copied from ClineProvider
        ...
        try {
            ...
            // Fallback logic copied from ClineProvider
    ...
    async updateServerTimeout(serverName: string, timeout: number): Promise<void> {
        if (this.mcpHub) {
            ...
            console.warn("ClineMcpManager: McpHub not available for updateServerTimeout");
        }
    }
    ...
    async deleteServer(serverName: string): Promise<void> {
        if (this.mcpHub) {
            ...
            console.warn("ClineMcpManager: McpHub not available for deleteServer");
        }
    }
    ...
    //     } else {
    //          console.warn("ClineMcpManager: McpHub not available for toggleToolAlwaysAllow");
    //     }
    // }
    ...
    async toggleServerDisabled(serverName: string, disabled: boolean): Promise<void> {
        if (this.mcpHub) {
            await this.mcpHub.toggleServerDisabled(serverName, disabled);
             ...
             console.warn("ClineMcpManager: McpHub not available for toggleServerDisabled");
        }
    }
    ...
    async restartConnection(serverName: string): Promise<void> {
        if (this.mcpHub) {
            await this.mcpHub.restartConnection(serverName);
             ...
             console.warn("ClineMcpManager: McpHub not available for restartConnection");
        }
    }
    ...
    async getMcpSettingsFilePath(): Promise<string | undefined> {
        ...
        console.warn("ClineMcpManager: McpHub not available for getMcpSettingsFilePath, constructing fallback path.");
     ...
     */
    getAllServers(): any[] { // Consider defining a proper type for ServerConfig if available
    ...
    //     } else {
    //          console.warn("ClineMcpManager: McpHub not available for registerServer");
    //          return false;
    //     }
    // }
    // --- End of commented out method ---
    ...
    dispose(): void {
        ...
        console.log("ClineMcpManager disposed (reference cleared).");
