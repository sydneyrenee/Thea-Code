...
describe("McpIntegration", () => {
	...
	describe("initialization", () => {
		it("should initialize the MCP tool router", async () => {
			...
			const mcpToolRouter = (mcpIntegration as unknown as { mcpToolRouter: { initialize: jest.Mock } })
				.mcpToolRouter
	...
	describe("tool registration", () => {
		it("should register a tool with the MCP tool system", () => {
			...
			const mcpToolSystem = (mcpIntegration as unknown as { mcpToolSystem: { registerTool: jest.Mock } })
				.mcpToolSystem
		...
		it("should unregister a tool from the MCP tool system", () => {
			...
			const mcpToolSystem = (mcpIntegration as unknown as { mcpToolSystem: { unregisterTool: jest.Mock } })
				.mcpToolSystem
	...
	describe("tool processing", () => {
		...
		it("should process XML tool use requests", async () => {
			...
			const mcpToolRouter = (mcpIntegration as unknown as { mcpToolRouter: { routeToolUse: jest.Mock } })
				.mcpToolRouter
		...
		it("should route tool use requests based on format", async () => {
			...
			const mcpToolRouter = (
				mcpIntegration as unknown as { mcpToolRouter: { detectFormat: jest.Mock; routeToolUse: jest.Mock } }
			).mcpToolRouter
...
describe("handleToolUse", () => {
	...
	it("should initialize the MCP integration if not already initialized", async () => {
		...
		const mcpToolRouter = (
			mcpIntegration as unknown as { mcpToolRouter: { initialize: jest.Mock; routeToolUse: jest.Mock } }
		).mcpToolRouter
