...
// Import McpConverters using a more absolute path
...
export class McpToolRouter extends EventEmitter {
	...
	private constructor(config?: SseTransportConfig) {
		...
		this.mcpToolSystem.on("started", (info: unknown) => this.emit("started", info))
		...
		switch (request.format) {
			...
			case ToolUseFormat.NEUTRAL:
				if (typeof request.content === "string") {
					...
					if (
						!(
							"type" in request.content &&
							"id" in request.content &&
